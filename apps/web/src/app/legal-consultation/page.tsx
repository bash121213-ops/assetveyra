    ...
    <main className="app-shell">
      ...
      <section className="form-page">
        ...
        <form onSubmit={submit} className="form-grid">
          ...
          <label><I18nText id="Legal matter type" /><select name="matter_type" defaultValue="real_estate" required>
            <option value="real_estate">{translate('Real estate', locale)}</option>
            <option value="transaction">{translate('Real estate transaction', locale)}</option>
            <option value="investment">{translate('Investment structure or documentation', locale)}</option>
            <option value="contract">{translate('Contract review or drafting', locale)}</option>
            <option value="due_diligence">{translate('Legal due diligence', locale)}</option>
            <option value="dispute">{translate('Dispute or claim', locale)}</option>
            <option value="corporate">{translate('Corporate or entity matter', locale)}</option>
            <option value="other">{translate('Other legal matter', locale)}</option>
          </select></label>
          <label><I18nText id="Your role" /><select name="role" defaultValue="investor" required>
            <option value="investor">{translate('Investor', locale)}</option>
            <option value="seller">{translate('Seller', locale)}</option>
            <option value="buyer">{translate('Buyer', locale)}</option>
            <option value="company">{translate('Company or organization', locale)}</option>
            <option value="other">{translate('Other', locale)}</option>
          </select></label>
          <label><I18nText id="Urgency" /><select name="urgency" defaultValue="standard" required>
            <option value="standard">{translate('Standard', locale)}</option>
            <option value="soon">{translate('Within a few days', locale)}</option>
            <option value="urgent">{translate('Urgent', locale)}</option>
          </select></label>
