...
type Asset = { id:string; title:string; asset_type:string; country_code:string|null; region:string|null; city:string|null; area_sqm:number|null; currency:string|null; asking_price:number|null; public_summary:string|null; property_details:unknown };
...
export default async function OpportunitiesPage(){
  ...
  if(!opportunityError&&opportunities?.length){
    ...
    if(assetIds.length){
      const result=await s.from('assets').select('id,title,asset_type,country_code,region,city,area_sqm,currency,asking_price,public_summary,property_details').in('id',assetIds).eq('status','published');
      assets=(result.data??[]) as Asset[];assetError=result.error;
      if(!assetError&&assets.length) imageMap=await getPublishedAssetImages(s,assets.map(asset=>asset.id));
    }
  }
  ...
  const externalListings=(externalResult.data??[]) as ExternalListing[];const assetById=new Map(assets.map(asset=>[asset.id,asset]));
  ...
  const error=opportunityError||assetError||externalResult.error;const grouped=externalCountryOrder.map(code=>({code,listings:externalListings.filter(item=>item.country_code===code)})).filter(group=>group.listings.length);
  return <main className="app-shell"><header className="app-header"><a className="brand" href="/">ASSETVEYRA</a><nav><a href="/opportunities"><I18nText id="Marketplace"/></a><a href="/workspace"><I18nText id="Overview"/></a>{user?<form action="/logout" method="post"><button className="text-button"><I18nText id="Sign out"/></button></form>:<a href="/login"><I18nText id="Sign in"/></a>}</nav></header>
    ...
    <section className="opportunity-grid">
      {rows.map(({opportunity,asset})=>{if(!asset)return null;const images=imageMap.get(asset.id)??[];const primary=images[0]?.signed_url;return <a className="opportunity-card" href={user?`/opportunities/${opportunity.slug}`:'/login'} key={opportunity.id}>
        {primary?<img src={primary} alt="" loading="lazy" style={{width:'100%',aspectRatio:'16 / 10',objectFit:'cover',display:'block'}}/>:<div className="empty-state" style={{minHeight:160}}><span><I18nText id="No listing image available"/></span></div>}
        {user?<><div className="card-meta"><span>{sectorKeys[asset.asset_type]?<I18nText id={sectorKeys[asset.asset_type]}/>:asset.asset_type}</span><span>{asset.country_code??'—'}</span></div><h2>{asset.title}</h2><p>{asset.public_summary||opportunity.investment_thesis||<I18nText id="Investment opportunity"/>}</p><PropertyDetailsSummary propertyDetails={asset.property_details}/><div className="card-data"><span>{asset.city||asset.region||'—'}</span><strong>{formatAmount(Number(asset.asking_price),asset.currency)}</strong></div></>:<><div className="card-meta"><span><I18nText id="Investment opportunity"/></span><span>{asset.country_code??'—'}</span></div><h2><I18nText id="Investment opportunity"/></h2><p><I18nText id="Sign in to view opportunity details and pricing."/></p><div className="card-data"><span><I18nText id="Access"/></span><strong><I18nText id="Sign in"/></strong></div></>}
      </a>})}
