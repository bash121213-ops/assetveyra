  ...
  return <main className="app-shell">
    ...
    <section className="opportunity-grid">
      {rows.map(({opportunity,asset})=>asset?<a className="opportunity-card" href={user?`/opportunities/${opportunity.slug}`:'/login'} key={opportunity.id}>
        {user?<><div className="card-meta"><span>{sectorKeys[asset.asset_type]?<I18nText id={sectorKeys[asset.asset_type]}/>:asset.asset_type}</span><span>{asset.country_code??'—'}</span></div><h2>{asset.title}</h2><p>{asset.public_summary||opportunity.investment_thesis||<I18nText id="Investment opportunity"/>}</p><div className="card-data"><span>{asset.city||asset.region||'—'}</span><strong>{formatAmount(Number(asset.asking_price),asset.currency)}</strong></div></>:<><div className="card-meta"><span><I18nText id="Investment opportunity"/></span><span>{asset.country_code??'—'}</span></div><h2><I18nText id="Investment opportunity"/></h2><p><I18nText id="Sign in to view opportunity details and pricing."/></p><div className="card-data"><span><I18nText id="Access"/></span><strong><I18nText id="Sign in"/></strong></div></>}
      </a>:null)}
      ...
      {grouped.map(group=><section className="country-market" key={group.code}>
        ...
        <div className="external-listing-grid">
          {group.listings.map(item=><article className="external-listing-card" key={item.id}>
            ...
            <div className="external-card-footer"><strong>{user?formatAmount(item.price_amount,item.currency):<ExternalMarketText id="Sign in to view pricing"/>}</strong><a href={user?`/contact?opportunity=${encodeURIComponent(item.title)}`:'/login'}><ExternalMarketText id={user?'Request this opportunity':'Sign in to request'}/></a></div>
