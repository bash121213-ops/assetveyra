...
async function registerInterest(formData: FormData) {
  ...
  if (!existing) {
    const { error } = await s.from('investor_interests').insert({ investor_organization_id: investor.organization_id, opportunity_id: opportunity.id, status: 'interest', created_by: user.id });
