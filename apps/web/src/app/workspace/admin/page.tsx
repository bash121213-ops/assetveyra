...
async function act(formData: FormData) {
  ...
  if (!reason) redirect('/workspace/admin?error=decision_reason_required');

  const { data: opportunity, error: oe } = await s
    .from('opportunities')
    .select('id,asset_id,status,visibility,published_at,owner_organization_id')
    .eq('id', id)
    .maybeSingle();
  ...
  const { data: verification } = await s
    ...
    .select('id,status,decision_reason,resolved_at')
  ...
  if (action === 'approve_compliance') {
    ...
    if (!verification || verification.status !== 'approved') redirect('/workspace/admin?error=verification_gate');

    const { error } = await s.from('opportunities').update({ status: 'approved' }).eq('id', id);
    if (error) throw new Error(error.message);

    if (opportunity.asset_id) {
      const { error: ae } = await s.from('assets').update({ status: 'approved' }).eq('id', opportunity.asset_id);
      if (ae) throw new Error(ae.message);
    }

    const { error: workflowError } = await s.from('workflow_events').insert({
      organization_id: opportunity.owner_organization_id,
      actor_id: userId,
      event_type: 'opportunity.compliance_approved',
      entity_type: 'opportunity',
      entity_id: id,
      payload: { reason, verification_case_id: verification.id },
    });
    ...
    const { error: auditError } = await s.from('audit_events').insert({
      organization_id: opportunity.owner_organization_id,
      actor_id: userId,
      action: 'opportunity.compliance_approved',
      entity_type: 'opportunity',
      entity_id: id,
      previous_state: { status: opportunity.status, asset_status: assetStatus },
      new_state: { status: 'approved', asset_status: 'approved' },
      metadata: { reason, verification_case_id: verification.id },
    });
    if (auditError) throw new Error(auditError.message);
  } else if (action === 'publish') {
    if (opportunity.status !== 'approved') redirect('/workspace/admin?error=publish_gate');
    if (assetStatus !== 'approved') redirect('/workspace/admin?error=asset_approval_gate');
    if (!verification || verification.status !== 'approved') redirect('/workspace/admin?error=verification_gate');

    const publishedAt = new Date().toISOString();
    const { error } = await s
      .from('opportunities')
      .update({ status: 'published', visibility: 'public', published_at: publishedAt })
      .eq('id', id);
    if (error) throw new Error(error.message);
    ...
    const { error: workflowError } = await s.from('workflow_events').insert({
      ...
      entity_type: 'opportunity',
      entity_id: id,
      payload: { reason, published_at: publishedAt, verification_case_id: verification.id },
    });
    if (workflowError) throw new Error(workflowError.message);
    ...
    const { error: auditError } = await s.from('audit_events').insert({
      ...
      entity_type: 'opportunity',
      entity_id: id,
      previous_state: { status: opportunity.status, visibility: opportunity.visibility, asset_status: assetStatus },
      new_state: { status: 'published', visibility: 'public', asset_status: 'published' },
      metadata: { reason, published_at: publishedAt, verification_case_id: verification.id },
    });
    if (auditError) throw new Error(auditError.message);
    ...
    const { error } = await s
      .from('opportunities')
      .update({ status: 'suspended', visibility: 'private' })
      .eq('id', id);
    if (error) throw new Error(error.message);

    if (opportunity.asset_id) {
      const { error: ae } = await s.from('assets').update({ status: 'suspended' }).eq('id', opportunity.asset_id);
      if (ae) throw new Error(ae.message);
    }
    ...
    const { error: workflowError } = await s.from('workflow_events').insert({
      organization_id: opportunity.owner_organization_id,
      actor_id: userId,
      event_type: 'opportunity.suspended',
      entity_type: 'opportunity',
      entity_id: id,
      payload: { reason },
    });
    ...
    const { error: auditError } = await s.from('audit_events').insert({
      ...
      entity_id: id,
      previous_state: { status: opportunity.status, visibility: opportunity.visibility, asset_status: assetStatus },
      new_state: { status: 'suspended', visibility: 'private', asset_status: 'suspended' },
      metadata: { reason },
    });
    if (auditError) throw new Error(auditError.message);
  ...
  return <main className="app-shell">
    ...
    <section className="panel"><h2>Opportunity control</h2><div className="table">{(opportunities ?? []).map((o: any) => <div className="row" key={o.id} style={{display:'grid',gap:10}}>
      ...
      <div style={{display:'grid',gap:8}}>
        {o.status === 'compliance_review' && <form action={act} style={{display:'grid',gap:6}}><input name="reason" required placeholder="Compliance decision rationale"/><input type="hidden" name="action" value="approve_compliance"/><input type="hidden" name="id" value={o.id}/><button className="button primary">Approve compliance</button></form>}
        {o.status === 'approved' && <form action={act} style={{display:'grid',gap:6}}><input name="reason" required placeholder="Publication decision rationale"/><input type="hidden" name="action" value="publish"/><input type="hidden" name="id" value={o.id}/><button className="button primary">Publish</button></form>}
        {['published','approved'].includes(o.status) && <form action={act} style={{display:'grid',gap:6}}><input name="reason" required placeholder="Suspension rationale"/><input type="hidden" name="action" value="suspend"/><input type="hidden" name="id" value={o.id}/><button className="button">Suspend</button></form>}
      </div>
