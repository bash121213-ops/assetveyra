...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ updated?: string; error?: string }> }) {
  ...
  return <main className="app-shell">
    ...
    <section className="page-head"><div className="eyebrow"><I18nText id="Operations"/></div><h1><I18nText id="Open platform controls"/></h1><p><I18nText id="Applicable administrative and workflow actions are recorded through existing audit mechanisms."/></p></section>
    {params.updated && <section className="panel"><strong><I18nText id="Your request has been received."/></strong></section>}
    {params.error && <section className="panel"><strong><I18nText id="Unable to send the request."/></strong></section>}
    <section className="panel"><h2><I18nText id="Opportunities"/></h2><div className="table">{(opportunities ?? []).map((o: any) => <div className="row" key={o.id} style={{display:'grid',gap:10}}>
      ...
      <span>{o.assets?.asking_price ? `${o.assets.asking_price} ${o.assets.currency || ''}` : <I18nText id="Pending"/>}</span>
      <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
        {o.status === 'compliance_review' && <form action={act}><input type="hidden" name="action" value="approve_compliance"/><input type="hidden" name="id" value={o.id}/><button className="button primary"><I18nText id="Approve verification"/></button></form>}
    ...
    </div>)}{!(opportunities?.length) && <div className="empty-state"><strong><I18nText id="No published opportunities yet"/></strong><span><I18nText id="No sample or fabricated inventory is displayed."/></span></div>}</div></section>
    <section className="panel"><h2><I18nText id="Contact"/></h2><div className="table">{(contacts ?? []).map((c: any) => <div className="row" key={c.id} style={{display:'grid',gap:8}}><div><strong>{c.name}</strong><div style={{color:'var(--muted)',fontSize:11,marginTop:5}}>{c.email} · {c.interest} · {c.status}</div></div><span>{c.message}</span>{c.status !== 'resolved' && <form action={act}><input type="hidden" name="action" value="resolve_contact"/><input type="hidden" name="id" value={c.id}/><button className="button"><I18nText id="Try again"/></button></form>}</div>)}{!(contacts?.length) && <div className="empty-state"><strong><I18nText id="No registered interests"/></strong><span><I18nText id="Information handled"/></span></div>}</div></section>
  </main>;
}
