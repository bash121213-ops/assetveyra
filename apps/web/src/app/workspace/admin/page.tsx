...
async function act(formData: FormData) {
  ...
  if (action === 'publish') {
    const { data: opportunity, error } = await s.from('opportunities').select('id,asset_id,status,owner_organization_id').eq('id', id).single();
    if (error || !opportunity || opportunity.status !== 'approved') redirect('/workspace/admin?error=publish_gate');
    const { error: oe } = await s.from('opportunities').update({ status: 'published', visibility: 'public', published_at: new Date().toISOString() }).eq('id', id);
    ...
    const { data: opportunity } = await s.from('opportunities').select('id,asset_id,owner_organization_id,status').eq('id', id).single();
    if (!opportunity) redirect('/workspace/admin?error=not_found');
    const { error: oe } = await s.from('opportunities').update({ status: 'suspended', visibility: 'private' }).eq('id', id);
    if (oe) throw new Error(oe.message);
...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ updated?: string; error?: string }> }) {
  ...
  return <main className="app-shell">
    ...
    <section className="page-head"><div className="eyebrow">OPERATIONS CONTROL</div><h1>Platform operations.</h1><p>Controlled publication, opportunity oversight and contact intake. Public publication requires an approved opportunity.</p></section>
    ...
    <section className="panel"><h2>Opportunity control</h2><div className="table">{(opportunities ?? []).map((o: any) => <div className="row" key={o.id} style={{display:'grid',gap:10}}>
      ...
      <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
        ...
        {o.status === 'published' && <form action={act}><input type="hidden" name="action" value="suspend"/><input type="hidden" name="id" value={o.id}/><button className="button">Suspend</button></form>}
      </div>
    ...
    <section className="panel"><h2>Contact intake</h2><div className="table">{(contacts ?? []).map((c: any) => <div className="row" key={c.id} style={{display:'grid',gap:8}}>
      <div><strong>{c.name}</strong><div style={{color:'var(--muted)',fontSize:11,marginTop:5}}>{c.email} · {c.interest} · {c.status}</div></div>
      <span>{c.message}</span>
      {c.status !== 'resolved' && <form action={act}><input type="hidden" name="action" value="resolve_contact"/><input type="hidden" name="id" value={c.id}/><button className="button">Mark resolved</button></form>}
