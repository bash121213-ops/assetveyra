...
export async function uploadAssetImages(assetId: string, formData: FormData) {
  ...
  for (const file of files) {
    ...
    const path = `org/${asset.organization_id}/asset/${asset.id}/${id}.${extension}`;
  ...
  if (rows.length) {
    ...
    if (error) {
      await s.storage.from('property-images').remove(uploadedPaths);
      throw new Error(error.message);
    }
  }
  revalidatePath(`/workspace/assets/${assetId}`);
  revalidatePath(`/workspace/assets/${assetId}/images`);
...
export async function deleteAssetImage(imageId: string) {
  ...
  revalidatePath(`/workspace/assets/${image.asset_id}/images`);
  revalidatePath('/workspace/assets');
}
