...
export default function AssetImageUploaderV2({ inputName = 'images' }: { inputName?: string }) {
  ...
  function onChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    const prepared = selected.map((file) => {
      if (!file.type.startsWith('image/')) return { file, preview: '', name: file.name, error: 'Unsupported image format.' };
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(120px,1fr))', gap: 10 }}>
            {images.map((image, index) => (
              <div key={`${image.name}-${index}`} style={{ border: '1px solid var(--border)', borderRadius: 8, padding: 6, background: 'var(--surface)', position: 'relative' }}>
                {image.preview ? (
                  <img src={image.preview} alt={image.name} style={{ width: '100%', aspectRatio: '1 / 1', objectFit: 'cover', borderRadius: 6, display: 'block' }} />
                ) : (
                  <div role="alert" style={{ aspectRatio: '1 / 1', display: 'grid', placeItems: 'center', color: 'var(--muted)', fontSize: 12 }}><I18nText id="Error" /></div>
                )}
