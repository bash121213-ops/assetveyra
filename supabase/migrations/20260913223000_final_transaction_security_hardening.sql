...
using (
  ...
  or exists (
    ...
    where dr.id = data_room_members.data_room_id and om.user_id = auth.uid()
  )
);
...
using (
  ...
  or exists (
    ...
    where om.organization_id = investor_interests.investor_organization_id
      and om.user_id = auth.uid()
      and om.role = any(array['investor_admin'::public.member_role,'platform_admin'::public.member_role,'operations_admin'::public.member_role,'deal_manager'::public.member_role])
  )
)
with check (
  created_by = auth.uid()
...
with check (
  ...
  and exists (
    ...
    join public.organization_members om on om.organization_id = ii.investor_organization_id
    where ii.id = offers.interest_id and om.user_id = auth.uid()
  )
);
...
using (
  ...
  or exists (
    select 1 from public.investor_interests ii
    join public.opportunities op on op.id = ii.opportunity_id
    join public.organization_members om on om.organization_id = op.owner_organization_id
    where ii.id = offers.interest_id and om.user_id = auth.uid()
  )
  or exists (
    select 1 from public.organization_members om
    where om.user_id = auth.uid() and om.role = any(array['platform_admin'::public.member_role,'operations_admin'::public.member_role,'deal_manager'::public.member_role])
  )
)
with check (
  submitted_by = auth.uid()
...
using (
  ...
  or exists (
    select 1 from public.data_rooms dr
    join public.data_room_members drm on drm.data_room_id = dr.id
    where dr.opportunity_id = documents.opportunity_id
      and drm.user_id = auth.uid()
      and (drm.expires_at is null or drm.expires_at > now())
      and ((not dr.nda_required) or exists (
        select 1 from public.nda_acceptances na
        where na.opportunity_id = dr.opportunity_id
          and na.organization_id = dr.organization_id
    ...
    where d.id = document_versions.document_id
      and (
        ...
        or exists (
          select 1 from public.data_rooms dr
          join public.data_room_members drm on drm.data_room_id = dr.id
          where dr.opportunity_id = d.opportunity_id
            and drm.user_id = auth.uid()
            and (drm.expires_at is null or drm.expires_at > now())
            and ((not dr.nda_required) or exists (
              select 1 from public.nda_acceptances na
